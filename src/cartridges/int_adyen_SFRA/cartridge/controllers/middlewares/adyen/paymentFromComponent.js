...
function handleGiftCard(currentBasket, order) {
...
function handleCancel(reqDataObj, res, next) {
...
function handleErrorResult(order, result, reqDataObj) {
...
function paymentFromComponent(req, res, next) {
  ...
  if (reqDataObj.cancelTransaction) {
    handleCancel(reqDataObj, res, next);
  }
  ...
  if (result.resultCode === constants.RESULTCODES.REFUSED) {
    handleErrorResult(order, result, reqDataObj);
  }

  // Check if gift card was used
  if (session.privacy.giftCardResponse) {
    handleGiftCard(currentBasket, order);
  }
