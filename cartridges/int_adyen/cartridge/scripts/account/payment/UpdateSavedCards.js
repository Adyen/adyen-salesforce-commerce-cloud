...
function updateSavedCards(args) {
    try {
        ...
        if (recurringPaymentsEnabled) {
            paymentsMap = getRecurringDetails(customer);
            if (paymentsMap != null && !paymentsMap.isEmpty() && paymentsMap.getLength() > 0) {
                ...
                Transaction.wrap(function () {
                    ...
                    for (var pspRef in paymentsMap) {
                        var payment = paymentsMap[pspRef];
                        ...
                        if ('cardType' in payment && !empty(payment.cardType)) {
                            var adyenCardType = payment.cardType;
                            ccType = AdyenHelper.setSFCCCardType(adyenCardType);
                        }
                        ...
                        if (!empty(expMonth) && !empty(expYear) && !empty(ccNum) && !empty(token) && !empty(ccType)) {
                            var newCreditCard = customer.getProfile().getWallet().createPaymentInstrument(PaymentInstrument.METHOD_CREDIT_CARD);

                            newCreditCard.custom.AdyenPspReference = pspRef;
