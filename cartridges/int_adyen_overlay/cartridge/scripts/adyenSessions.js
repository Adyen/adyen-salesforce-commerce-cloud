...
function createSession(basket, customer, countryCode) {
  try {
    var service = AdyenHelper.getService(AdyenHelper.SERVICE.SESSIONS);

    if (!service) {
      throw new Error('Could not do /sessions call');
    }

    var paymentAmount = AdyenHelper.getCurrencyValueForApi(basket.getTotalGrossPrice());
    var currencyCode = basket.currencyCode; //TODO: Change AdyenHelper so that this object can have a different name. Not creating a payment request here
    ...
    var sessionsRequest = AdyenHelper.createShopperObject({
      order: basket,
