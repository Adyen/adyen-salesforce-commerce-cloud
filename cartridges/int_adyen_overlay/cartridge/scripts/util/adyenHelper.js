...
var _require = require('*/cartridge/scripts/config/blockedPaymentMethods.json'),
    blockedPaymentMethods = _require.blockedPaymentMethods;
...
var adyenHelperObj = {
  ...
  getAmazonMerchantID: function getAmazonMerchantID() {
    return adyenHelperObj.getCustomPreference('Adyen_AmazonMerchantID');
  },
  getAmazonPublicKeyID: function getAmazonPublicKeyID() {
    return adyenHelperObj.getCustomPreference('Adyen_AmazonPublicKeyID');
  },
  getAmazonStoreID: function getAmazonStoreID() {
  ...
  createAdyenCheckoutResponse: function createAdyenCheckoutResponse(checkoutresponse) {
    if (['Authorised', 'Refused', 'Error', 'Cancelled'].indexOf(checkoutresponse.resultCode) !== -1) {
      return {
        isFinal: true,
    ...
    if (['RedirectShopper', 'IdentifyShopper', 'ChallengeShopper', 'PresentToShopper', 'Pending'].indexOf(checkoutresponse.resultCode) !== -1) {
      return {
        isFinal: false,
