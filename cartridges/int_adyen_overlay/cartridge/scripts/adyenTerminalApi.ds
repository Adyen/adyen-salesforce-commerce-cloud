...
function createTerminalPayment(order, paymentInstrument, terminalId) {
    try {
        ...
        if (!order || !paymentInstrument) {
            throw new Error("Could not retrieve payment data, order = " + JSON.stringify(order) + ", paymentInstrument = " + JSON.stringify(paymentInstrument));
        }
        ...
        if(paymentResult.error){
            throw new Error("Error in POS payment result: " + JSON.stringify(paymentResult.response));
        }
        else {
            ...
            throw new Error("No correct response: " + JSON.stringify(paymentResponse.Response));
        }
    }
    catch (e) {
