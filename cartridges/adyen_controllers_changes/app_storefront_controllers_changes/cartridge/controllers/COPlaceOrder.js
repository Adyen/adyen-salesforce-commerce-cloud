...
function start() {
  ...
  var COShipping = app.getController('COShipping'); // Clean shipments.

  COShipping.PrepareShipments(cart); // Make sure there is a valid shipping address, accounting for gift certificates that do not have one.
  ...
  var URLUtils = require('dw/web/URLUtils'); // Cache current order number in order to potentially restore cart.


  session.privacy.currentOrderNumber = order.orderNo;
  session.privacy.currentOrderToken = order.orderToken;
  var submitOrder = handlePaymentsResult.isAdyen === false || //adyen is not the payment processor for this payment
...
function clearForms() {
  ...
  session.forms.billing.clearFormElement(); // clear cached order number
