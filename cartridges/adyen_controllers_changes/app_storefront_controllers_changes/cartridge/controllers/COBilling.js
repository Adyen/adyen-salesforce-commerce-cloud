...
var guard = require('~/cartridge/scripts/guard'); // ### Custom Adyen cartridge start ###
...
var constants = require("*/cartridge/adyenConstants/constants"); // ### Custom Adyen cartridge end ###
...
function returnToForm(cart, params) {
  var pageMeta = require('~/cartridge/scripts/meta'); // if the payment method is set to gift certificate get the gift certificate code from the form
...
function publicStart() {
  ...
  if (cart) {
    ...
    initEmailAddress(cart); // Get the Saved Cards from Adyen to get latest saved cards
    ...
    var AdyenSessionsResponse = AdyenController.Sessions(customer); // var AdyenPosTerminals = AdyenController.GetTerminals();
    //TODO fix terminals
...
function handleCoupon() {
  var CouponError; // @FIXME what is that used for?
...
function billing(data) {
  ...
  if (session.privacy.currentOrderNumber && session.privacy.currentOrderToken) {
    var order = OrderMgr.getOrder(session.privacy.currentOrderNumber, session.privacy.currentOrderToken); // Clear cache so the order restore will only be attmpted once per order
  ...
  app.getForm('billing').handleAction({
    applyCoupon: function () {
      var couponCode = request.httpParameterMap.couponCode.stringValue || request.httpParameterMap.dwfrm_billing_couponCode.stringValue; // TODO what happened to this start node?
...
function selectCreditCard() {
  ...
  cart = app.getModel('Cart').get();
  applicableCreditCards = initCreditCardList(cart).ApplicableCreditCards;
  selectedCreditCard = null; // ensure mandatory parameter 'CreditCardUUID' and 'CustomerPaymentInstruments'
...
function saveCreditCard() {
  if (AdyenConfigs.getAdyenRecurringPaymentsEnabled()) {
    ...
    if (customer.authenticated && app.getForm('billing').object.paymentMethods.creditCard.saveCard.value) {
      ...
      Transaction.wrap(function () {
        newCreditCard = customer.getProfile().getWallet().createPaymentInstrument(PaymentInstrument.METHOD_CREDIT_CARD); // copy the credit card details to the payment instrument
