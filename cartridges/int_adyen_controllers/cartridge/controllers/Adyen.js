...
function showConfirmation() {
  try {
    ...
    var order = OrderMgr.getOrder(merchantReference, orderToken); // if the payment is authorized, we can navigate to order confirm
    ...
    if (adyenPaymentInstrument.paymentTransaction.custom.Adyen_merchantSig === signature) {
      ...
      var hasQuerystringDetails = !!(details.redirectResult || details.payload); // Saved response from Adyen-PaymentsDetails
...
function paymentsDetails() {
  try {
    ...
    if (paymentsDetailsResponse.merchantReference !== 'recurringPayment-account') {
      ...
      _response.redirectUrl = URLUtils.url('Adyen-ShowConfirmation', 'merchantReference', _response.merchantReference, 'signature', signature, 'orderToken', order.orderToken).toString();
    }
...
function paymentFromComponent() {
  if (request.httpParameterMap.getRequestBodyAsString().indexOf('cancelTransaction') > -1) {
    ...
    Transaction.commit(); // Decline flow for Amazon pay is handled different from other Component PMs
